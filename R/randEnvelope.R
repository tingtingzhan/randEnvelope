...
randEnvelope <- function(
  ...
  noout_ <- lapply(seq_len(length.out = n), FUN = function(i) {
    ...
    print(p)
    if (!(i %% 10L)) message('\r', i, ' out of ', n, ' envelopes printed', appendLF = FALSE)
  })
  dev.off()
  ...
  noout_ <- lapply(seq_len(length.out = n), FUN = function(i) {
    ...
    print(p)
    if (!(i %% 10L)) message('\r', i, ' out of ', n, ' inserts printed', appendLF = FALSE)
  })
  dev.off()
  message('\r', n, ' out of ', n, ' inserts printed', appendLF = TRUE)
