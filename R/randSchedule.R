...
randSchedule <- function(
  ...
  txt_ratio <- sprintf(
    fmt = 'block size multipliers of %s are permuted. A %s allocation ratio is applied to the %d arms of %s within each block.', 
    paste0('`\u00d7', mblock, '`', collapse = ' and '), 
    paste(tx_ratio, collapse = ':'), 
    length(treatment),
    paste0('`', treatment, '`', collapse = ' and '))
  ...
  txt_strata <- if (nstrata) {
    sprintf(
      ...
      n)
  } else sprintf(fmt = 'A total of %d records are generated.', n)
  
  out_txt <- paste(
    capitalize(txt_nm), 'randomization schedule is generated using [R].', 
