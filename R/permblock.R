...
#' 
#' @slot treatment \link[base]{character} \link[base]{vector}, name of treatments, 
...
setMethod(f = initialize, signature = 'permblock', definition = function(.Object, ...) {
  ...
  if (!length(x@ratio)) {
    x@ratio <- rep(1L, times = length(x@treatment))
  }
...
setValidity(Class = 'permblock', method = function(object) {
  ...
  if (length(ratio) && (ntx != length(ratio))) stop('`@treatment` and `@ratio` must be same length')
